...
[Autogenerated build](https://github.com/saulshanabrook/pypi-dockerhub)
of [%[1]v PyPi package](https://pypi.python.org/pypi/%[1]v), based on the
`+"`python:3`"+` image.

Using the `+"`latest'"+` branch will pull in the last version built.

I reccomend pinning to an explicit version number, because it is possible
if an old version is built retroactively that it will be set to
`+"`latest'"+`.
